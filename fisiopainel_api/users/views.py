...
class PacoteViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        # Capturar valores antigos ANTES de salvar
        old_data_inicio = instance.data_inicio
        old_dias_semana = instance.dias_semana
        old_horario = instance.horario_atendimento
        old_quantidade = instance.quantidade_total

        # Obter novos valores da requisição
        ...
        # Detectar se houve mudança nos parâmetros de agendamento (comparando com os valores antigos capturados)
        mudou_agendamento = (
            (nova_data_inicio is not None and nova_data_inicio != old_data_inicio) or
            (novos_dias_semana is not None and novos_dias_semana != old_dias_semana) or
            (novo_horario is not None and novo_horario != old_horario)
        )
        ...
        # Caso a quantidade mudou, mas NÃO houve mudança nos dias/data
        elif nova_quantidade is not None and nova_quantidade != old_quantidade:
            total_atual = pacote.agendamentos.count()
            # ... resto da lógica de quantidade mantida ...
